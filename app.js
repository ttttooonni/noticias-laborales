...
function relativeDate(v) {
  const d = new Date(v), diff = Math.max(0, Date.now() - d.getTime()), m = Math.floor(diff / 60000);
  if (m < 60) return 'Hace ' + m + ' min';
  const h = Math.floor(m / 60);
  if (h < 24) return 'Hace ' + h + ' h';
  return new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'short' }).format(d);
}
function isLast24h(v) { return Date.now() - new Date(v).getTime() <= 24 * 60 * 60 * 1000; }
function loadWatch() {
...
async function loadNews(force = false) {
  try {
    ...
    health = data.health || { failures: [] };
    document.getElementById('lastUpdate').textContent = data.updated_at ? formatDate(data.updated_at) : 'Actualizado';
    render();
  } catch (e) {
    if (!news.length) grid.innerHTML = '<div class="loading">No se han podido cargar las noticias. Comprueba la conexión.</div>';
  }
}

function matches(n, filter) {
  if (filter === 'Todas') return true;
  if (filter === 'Última hora') return isLast24h(n.published);
  if (filter === 'UGT') return n.source_type === 'UGT' || (n.categories || []).includes('UGT');
  return (n.categories || []).includes(filter);
}

function render() {
  const filtered = news.filter((n) => matches(n, activeFilter));
  ...
  const text = latest.map((n) => `<span>• ${escapeHtml(n.title)}</span>`).join('');
  document.getElementById('tickerTrack').innerHTML = text + text;
  renderPulse();
  renderHealth();
}
...
function renderHealth() {
  ...
  if (!box) {
    ...
    if (pulse) pulse.insertAdjacentElement('beforebegin', box);
  }
  const fails = health.failures || [];
  box.innerHTML = fails.length
    ? `<div class="loading" style="text-align:left">Algunas fuentes no respondieron:<br>${fails.map((f) => escapeHtml(f.source) + ': ' + escapeHtml(f.error)).join('<br>')}</div>`
    : '';
}
...
function card(n, i) {
  ...
  const pin = n.pin ? ' · fijada' : '';
  return `<article class="${cls}" data-id="${escapeAttr(n.id)}"><div class="meta"><span class="badge ${badgeClass}">${escapeHtml(n.source_label || 'FUENTE')}</span><small>${relativeDate(n.published)}${pin}</small></div><h3>${escapeHtml(n.title)}</h3><p>${escapeHtml(n.summary || '')}</p><div class="source">${escapeHtml(n.source)} <span class="read">Leer →</span></div></article>`;
}

function section(title, text) {
  return text ? `<div class="article-section"><h4>${title}</h4><p>${escapeHtml(text)}</p></div>` : '';
}
...
function openArticle(id) {
  ...
  const shareText = [n.title, n.impact || n.summary, href].filter(Boolean).join('\n\n');
  const wa = 'https://wa.me/?text=' + encodeURIComponent(shareText);
  articleContent.innerHTML = `<button class="article-close" aria-label="Cerrar">×</button><span class="badge ${n.source_type === 'UGT' ? 'ugt' : isSentence ? 'sentencia' : ''}">${escapeHtml(n.source_label || 'FUENTE')}</span><h2 class="article-title">${escapeHtml(n.title)}</h2><p class="source">${escapeHtml(n.source)} · ${formatDate(n.published)}</p>${section('¿Qué ha pasado?', n.what_happened)}${section('¿A quién afecta?', n.who_affected)}${section('¿Qué significa?', n.impact)}${n.ugt_position ? section('Posición UGT', n.ugt_position) : ''}<div class="article-section"><h4>Fuente original</h4><p>Consulta siempre el documento original.</p>${href ? `<a class="source-link" href="${escapeAttr(href)}" target="_blank" rel="noopener noreferrer">Ver fuente original →</a>` : ''}<a class="source-link" style="margin-left:8px;background:#128c7e" href="${escapeAttr(wa)}" target="_blank" rel="noopener noreferrer">WhatsApp</a></div>`;
  articleContent.querySelector('.article-close').onclick = () => dialog.close();
  dialog.showModal();
}
...
const topicNames = ['Todas', 'Última hora', 'Derechos laborales', 'Sentencias', 'Convenios', 'Salarios', 'Empleo', 'Canarias', 'Las Palmas', 'Hostelería', 'UGT'];
...
window.addEventListener('beforeinstallprompt', (e) => {
  ...
  btn.hidden = false;
  btn.onclick = async () => { await deferredPrompt.prompt(); deferredPrompt = null; btn.hidden = true; };
});
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('./sw.js').then((reg) => {
    if (reg.waiting) reg.waiting.postMessage({ type: 'SKIP_WAITING' });
  }).catch((err) => console.warn('SW no instalado', err));
}
loadNews();
setInterval(() => loadNews(true), REFRESH_MS);
